...
Word *_DObject_New(byte *name, uint32 value, uint64 ctype, uint64 ltype, uint64 ftype, byte *function, int arg, int32 addToInNs, Namespace *addToNs, uint64 allocType);
...
void _Lexer_ParseAsAString(Lexer *lexer, int32 allocType);
void Lexer_ParseString(Lexer *lexer);
void _Lexer_ParseBinary(Lexer *lexer, int offset);
void Lexer_ParseBinary(Lexer *lexer, byte *token, int32 allocType, int32 offset);
void Lexer_ParseBigNum(Lexer *lexer, byte *token);
void _Lexer_ParseHex(Lexer *lexer, byte *token, int32 allocType);
void _Lexer_ParseDecimal(Lexer *lexer, byte *token, int32 allocType);
...
byte *_Mem_Allocate(int32 size, uint64 type, int32 flags);
...
void NBA_FreeChunkType(Symbol *s, uint64 type, int32 exactFlag);
...
byte *_Lexer_ParseNextToken_WithDelimiters(Lexer *lexer, byte *delimiters, int32 checkListFlag, uint64 state);
...
void Lexer_Init(Lexer *lexer, byte *delimiters, uint64 state, int32 allocType);
...
void _Namespace_DoSetState(Namespace *ns, uint64 state);
...
void _CfrTil_Contex_NewRun_2(CfrTil *cfrTil, ContextFunction_2 contextFunction, byte *arg, int32 arg2, int32 allocType);
...
ByteArray *_ByteArray_Allocate(int32 size, int64 type);
ByteArray *ByteArray_Init(ByteArray *ba, int32 size, int64 type);
ByteArray *_ByteArray_AllocateNew(int32 size, int64 type);
ByteArray *ByteArray_AllocateNew(int32 size, int64 type);
...
CompileOptimizer *CompileOptimizer_New(Compiler *compiler, int32 type);
...
/* core/finder.c */
Word *_WordList_DescendMap_1(Word *word, uint64 state, int32 stayInOneNamespaceFlag, MapFunction_Cell_1 mf, int32 one);
...
int LC_Clear(LambdaCalculus *lc);
