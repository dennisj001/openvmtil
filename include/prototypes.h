...
int32 _GetWordStackState(int count);
...
void Class_Object_Init(Word *word, Namespace *ns);
...
Word *Class_Value_New(byte *name);
...
void CfrTil_Set_DebugSourceCodeIndex(Word *word);
...
void Lexer_ParseBinary(Lexer *lexer, byte *token, uint32 allocType, int32 offset);
...
Context *_Context_New(CfrTil *cfrTil, uint32 allocType);
...
Context *CfrTil_Context_PushNew(CfrTil *cfrTil, uint32 allocType);
void CfrTil_Context_PopDelete(CfrTil *cfrTil);
void _CfrTil_Contex_NewRun_1(CfrTil *cfrTil, ContextFunction_1 contextFunction, byte *arg, uint32 allocType);
void _CfrTil_Contex_NewRun_2(CfrTil *cfrTil, ContextFunction_2 contextFunction, byte *arg, int32 arg2, uint32 allocType);
void _CfrTil_Contex_NewRun_Void(CfrTil *cfrTil, Word *word, uint32 allocType);
void _Context_InterpretString(Context *cntx, byte *str);
void _CfrTil_ContextNew_InterpretString(CfrTil *cfrTil, byte *str, uint32 allocType);
...
Word *_Word_InitBasic(Word *word, uint64 ctype, uint64 ltype);
...
void Debugger_fflush(void);
void Debugger_SetupReturnStackCopy(Debugger *debugger, int32 size);
void Debugger_CompileAndDoInstruction(Debugger *debugger, byte *jcAddress);
...
void CfrTil_PropertydefStructBegin(void);
void CfrTil_PropertydefStructEnd(void);
void _Property_Create(void);
void _CfrTil_PropertyDef(void);
/* primitives/cmaths.c */
