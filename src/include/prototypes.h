...
void _Compile_CpuState_Restore(Cpu *cpu, int32 cStackRegFlag);
...
int32 Block_CopyCompile_WithLogicFlag(byte *srcAddress, int32 bindex, int32 jccFlag, int negFlag);
int32 Block_CopyCompile(byte *srcAddress, int32 bindex, int32 jccFlag);
...
Word *TC_Tree_Map_1(TabCompletionInfo *tci, dllist *list, MapFunction mf, Word *one, int32 *startFlag);
...
int32 _Lexer_ConsiderDebugAndCommentTokens(byte *token, int32 evalFlag, int32 addFlag);
byte *_Lexer_NextNonDebugOrCommentTokenWord(Lexer *lexer, int32 evalFlag);
byte *Lexer_PeekNextNonDebugTokenWord(Lexer *lexer, int32 evalFlag);
...
byte *Debugger_ShowSourceCodeLine(Debugger *debugger, Word *word, byte *token0, int32 twAlreayUsed);
...
void LO_Debug_ExtraShow(int32 showStackFlag, int32 verbosity, int32 wordList, byte *format, ...);
...
/* basis/sourceCode.c */
node *DWL_Check_SCWI(dlnode *node, byte *address, int32 scwi, Word *wordn, dlnode **maybeFoundNode, int32 *diff1);
dlnode *DWL_Find(Word *word, byte *address, byte *name, int32 fromFirst, int32 takeFirstFind, byte *newAddress);
...
void _CfrTil_WordLists_PopWord(int32 n);
...
void DWL_ShowNode(dlnode *node, byte *insert, int32 scwiDiff);
void DebugWordList_Show(void);
byte *PrepareSourceCodeString(byte *sc, Word *word, int32 scwi0);
...
void Debugger_Disassemble(Debugger *debugger, byte *format, byte *address, int32 cflag);
...
void Debugger_State_CheckSaveShow_UdisOneInsn(Debugger *debugger);
