 ...
 * 
 */
const int32 SC_WINDOW = 60 ;
const int32 SCWI_MAX_DIFF = 60 ;
const int32 SCWI_MIN_DIFF = 2 ;

node *
DWL_Check_SCWI ( dlnode * node, byte * address, int32 scwi, Word * wordn, dlnode ** maybeFoundNode, int32 * diff1 )
{
    dlnode *foundNode = 0 ;
    int32 scwiDiff ;
...
DWL_Find ( Word * word, byte * address, byte* name, int32 fromFirst, int32 takeFirstFind, byte * newAddress )
{
    ...
    int32 numFound = 0 ;
    uint32 wdiff = SC_WINDOW, fDiff, minDiffFound = SCWI_MAX_DIFF, scwi ; //, scwi0 = - 1 ;
            ...
            node = fromFirst ? dlnode_Next ( node ) : dlnode_Previous ( node ) )
        {
            ...
            if ( address == naddress )
            {
                ...
                if ( newAddress ) //&& ( address == caddress ) )
                {
                    ...
                    if ( _Q_->Verbosity > 2 ) _Printf ( "\nDWL_Find : ADDRESS ADJUST : word = \'%-12s\' : address = 0x%8x : scwi = %4d : newAddress = 0x%8x", nword->Name, address, scwi, newAddress ) ;
                    continue ;
                }
                //foundNode = node ;
        ...
        dobject_Set_M_Slot ( ( dobject* ) dobj, SCN_SC_CADDRESS, Here ) ;
        {
            ...
            if ( ( list = _CfrTil_->DebugWordList ) ) //GetState ( _CfrTil_, DEBUG_SOURCE_CODE_MODE ) )
            {
                ...
                for ( index = 0, node = dllist_First ( ( dllist* ) list ) ; node ; node = nextNode, index ++ )
                {
                    ...
                    if ( address == Here ) dlnode_Remove ( node ) ; // only the last at an address is correct
                }
            }
        }
...
void
_CfrTil_WordLists_PopWord ( int32 n )
{
    while ( n -- )
    {
        CfrTil_WordList_Pop ( ) ;
...
void
DWL_ShowNode ( dlnode * node, byte * insert, int32 scwiDiff )
{
    if ( node )
    {
...
DebugWordList_Show ( )
{
    ...
    if ( Is_DebugOn && ( list = _CfrTil_->DebugWordList ) ) //GetState ( _CfrTil_, DEBUG_SOURCE_CODE_MODE ) )
    {
        ...
        for ( index = 0, node = dllist_First ( ( dllist* ) list ) ; node ; node = nextNode, index ++ )
        {
            ...
            _Printf ( "\nDebugWordList : index %2d : word = \'%-12s\' : address = 0x%8x : scwi = %3d", index, word->Name, address, scwi ) ;
        }
        //_Pause ( ) ;
    }
}
