...
Literal_New ( Lexer * lexer, uint32 uliteral )
{
    ...
    else
    {
        if ( lexer->TokenType & ( T_STRING | T_RAW_STRING ) )
        {
            ...
            uliteral = ( int32 ) String_New ( lexer->LiteralString, Compiling ? STRING_MEM : TEMPORARY ) ;
        }
        name = lexer->OriginalToken ;
    }
    word = _DObject_New ( name, ( uint32 ) uliteral, LITERAL | CONSTANT | IMMEDIATE, 0, LITERAL, ( byte* ) _DataObject_Run, 0, 0, 0, TEMPORARY ) ;
    return word ;
}
