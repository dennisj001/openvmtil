...
void
Compile_Store ( Compiler * compiler, int64 stackReg, int64 size ) // !
{
    ...
    else if ( optFlag )
    {
        ...
        if ( compiler->optInfo->OptimizeFlag & OPTIMIZE_IMM ) _Compile_MoveImm ( compiler->optInfo->Optimize_Dest_RegOrMem,
            compiler->optInfo->Optimize_Rm, 0, compiler->optInfo->Optimize_Disp, compiler->optInfo->Optimize_Imm, size ) ;
        else if ( compiler->optInfo->OptimizeFlag & OPTIMIZE_REGISTER )
        {
            // allow for one of these to be EAX which is 0
            if ( compiler->optInfo->Optimize_SrcReg || compiler->optInfo->Optimize_DstReg ) _Compile_Move_Reg_To_Reg ( compiler->optInfo->Optimize_DstReg, compiler->optInfo->Optimize_SrcReg, size ) ;
                //_Compile_Move ( int64 direction, int64 reg, int64 rm, int64 sib, int64 disp )
            else _Compile_Move ( compiler->optInfo->Optimize_Dest_RegOrMem, compiler->optInfo->Optimize_Reg, compiler->optInfo->Optimize_Rm, 0, 0, size ) ;
        }
        else _Compile_Move_Reg_To_Rm ( compiler->optInfo->Optimize_Rm, compiler->optInfo->Optimize_Reg, compiler->optInfo->Optimize_Disp, size ) ;
    }
    else
    {
        Word * word ;
        if ( ( word = ( Word* ) Compiler_WordList ( 0 ) ) && word->StackPushRegisterCode ) SetHere ( word->StackPushRegisterCode ) ;
        else _Compile_Move_Rm_To_Reg ( EAX, stackReg, 0, size ) ;
        _Compile_Move_Rm_To_Reg ( ECX, stackReg, - CELL_SIZE, size ) ;
        _Compile_Move_Reg_To_Rm ( EAX, ECX, 0, size ) ;
...
void
Compile_Poke ( Compiler * compiler, int64 stackReg, int64 size ) // =
{
    ...
    {
        _Compile_Move_Rm_To_Reg ( ECX, stackReg, 0, size ) ;
        _Compile_Move_Rm_To_Reg ( EAX, stackReg, - CELL_SIZE, size ) ;
        _Compile_Move_Reg_To_Rm ( EAX, ECX, 0, size ) ;
...
void
Compile_AtEqual ( int64 stackReg, int64 size ) // !
{
    _Compile_Move_Rm_To_Reg ( EAX, stackReg, 0, size ) ;
    _Compile_Move_Rm_To_Reg ( EAX, EAX, 0, size ) ;
    _Compile_Move_Rm_To_Reg ( ECX, stackReg, - CELL_SIZE, size ) ;
    _Compile_Move_Reg_To_Rm ( ECX, EAX, 0, size ) ;
