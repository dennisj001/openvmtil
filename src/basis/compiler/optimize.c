...
{
  Word *w = ( node->afterWord && node->afterWord->afterWord ? (Word*) dobject_Get_M_Slot ( node, 0 ) : 0 ) ;
  //if ( w && (w->S_WordData) && (w->W_OriginalWord != w ) && (w->S_CProperty & RECYCLABLE_COPY)  )   
  if ( w && ( w->S_CProperty & RECYCLABLE_COPY ) ) //&& ( !( w->S_CProperty & ( CFRTIL_WORD | CPRIMITIVE | ALIAS ) ) ) )
  {
      //if ( String_Equal ( "d:", w->Name ) ){
      //node = (Node*) w ;
      //if ( ( node->afterWord == 0 ) && ( node->beforeWord == 0 ) )
      {

          d0 ( _Printf ( (byte*) "\nrecycling : %s", w->Name )  ) ;
          //d0 ( _Printf ( " %d", w->S_CProperty & ( CFRTIL_WORD | CPRIMITIVE ) ) ) ;
          Word_Recycle ( w ) ;
      }
  }
}
